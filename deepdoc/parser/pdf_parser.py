...
from rag.tokenizer.rag_tokenizer import RagTokenizer
from rag.config import PARALLEL_DEVICES
...
class RAGFlowPdfParser:
    def __init__(self, **kwargs):
        ...
        if PARALLEL_DEVICES > 1:
            self.parallel_limiter = [
                trio.CapacityLimiter(1) for _ in range(PARALLEL_DEVICES)
            ]
    ...
    def _assign_column(self, boxes, zoomin=3):
        ...
        for pg, bxs in by_page.items():
            ...
            logging.info(
                f"[Page {pg}] median_w={median_w:.2f}, page_w={page_w:.2f}, raw_cols={raw_cols}, cand={cand}"
            )
    ...
    def _naive_vertical_merge(self, zoomin=3):
        ...
        self.boxes = sorted(
            merged_boxes, key=lambda x: (x["page_number"], x.get("col_id", 0), x["top"])
        )
    ...
    def __images__(self, fnm, zoomin=3, page_from=0, page_to=299, callback=None):
        ...
        try:
            with sys.modules[LOCK_KEY_pdfplumber]:
                with (
                    ...
                    else pdfplumber.open(BytesIO(fnm)) as pdf
                ):
                    self.pdf = pdf
                    self.page_images = [
            ...
            if limiter:
                async with limiter:
                    await trio.to_thread.run_sync(
                        lambda: self.__ocr(i + 1, img, chars, zoomin, id)
                    )
            else:
        ...
        logging.info(
            f"__images__ {len(self.page_images)} pages cost {timer() - start}s"
        )
    ...
    def parse_into_bboxes(self, fnm, callback=None, zoomin=3):
        ...
        for b in self.boxes:
            ...
            b["positions"] = [
                [pos[0][-1] + 1, *pos[1:]]
                for pos in RAGFlowPdfParser.extract_positions(b["position_tag"])
            ]
...
if __name__ == "__main__":
    file_path = "D:/study/Thesis/3-Code/ads-rag/data/raw/pdf/Foundation Models in Autonomous Driving A Survey on Scenario Generation and Scenario Analysis.pdf"
    blob = open(file_path, "rb").read()
    bboxes = RAGFlowPdfParser().parse_into_bboxes(blob, callback=None)
    print(bboxes)
    export_json(
        output_dir="data/processed/pdf",
        file_name="Foundation Models in Autonomous Driving A Survey on Scenario Generation and Scenario Analysis.pdf.bboxes",
        content={"bboxes": bboxes},
    )

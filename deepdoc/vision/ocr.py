...
def load_model(model_dir, nm, device_id: int | None = None):
    ...
    model_cached_tag = (
        model_file_path + str(device_id) if device_id is not None else model_file_path
    )
    ...
    if cuda_is_available():
        ...
        run_options.add_run_config_entry(
            "memory.enable_memory_arena_shrinkage", "gpu:" + str(provider_device_id)
        )
        logging.info(
